...
class RepCounter(object):
    ...
    def process_measurements(self, time, x, y, dx, dy, norm_plate_height, norm_plate_width):
        ...
        if self.current_phase != Phase.HOLD:
            self._append_to_bar_path(x, y, norm_plate_width, norm_plate_height, time)
    ...
    def end_phase(self):
        ...
        if y_diff > self.max_y_diff * self.diff_threshold:
            ...
            for i in range(start_idx + 1, end_idx + 1):
                dx = abs(self.xs[i] - self.xs[i-1]) / ((self.widths[i] + self.widths[i-1]) / 2) * self.plate_diameter
                dy = abs(self.ys[i] - self.ys[i-1]) / ((self.heights[i] + self.heights[i-1]) / 2) * self.plate_diameter
